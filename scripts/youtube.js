...
function onYouTubeApiLoad() {
  ...
  gapi.client.setApiKey('AIzaSyDMOiRCizRZf5Rw-BeyHIrb6vkWEfFg638');
...
function displayVideo(result, i) {
  ...
  function onPlayerReady(e) {
    ...
    if (duration === 0) {
      //console.log(`Video ${myId} cannot be played, so it was deleted. event : ${e.target.id}`);
      //const vid = document.querySelector(`#vid${myId}`)
      //console.log(`vid : ${vid}`);
      //vid.parentNode.remove();
      //section.removeChild(vid);
      console.log('Duration is 0s')
    } else {
